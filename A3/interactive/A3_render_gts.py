...
import sys
sys.path.append('/Users/theoghanem/Dev/ECSE446_Realistic_Image_Synthesis/A3')
...
def main():
    ...
    if execute_modes['uniform']:
        ...
        for spp in spps:
            renderer.reset()
            for _ in tqdm(range(spp), desc='Rendering Uniform Images'):
                renderer.render()
            ...
            title = "uniform_veach_{x}spp.png".format(x=spp)
            plt.imshow(img)
            plt.axis('off')
            plt.savefig(title)
            plt.clf()

    if execute_modes['brdf']:
